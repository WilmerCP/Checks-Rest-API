...
let descriptions = {
    ...
    'delete user {id}': 'Deletes a specific user from database',
    ...
    'delete logs': 'Deletes all compressed log files'
...
e.on('delete user', function (str) {

    cli.responders.deleteUser(str);

});

e.on('delete logs', function (str) {

    cli.responders.deleteLogs();
...
cli.responders.deleteLogs = function () {


    console.log('Are you sure you want to delete all log history?');
    interface.removeListener('line', lineListener);
    interface.on('line', deleteLogsListener);
    interface.prompt();


}

cli.responders.deleteUser = function (str) {
    ...
    if (id) {
        ...
        interface.removeListener('line', lineListener);
        interface.on('line', deleteUserListener);
        interface.prompt();
        toDelete = id;
...
let deleteLogsListener = function (str) {
    ...
    if (str == 'yes') {
        ...
        _log.list(true, (error, logList) => {
            ...
            if (!error && logList && logList.length > 0) {
                ...
                for (let i = logList.length - 1; i >= 0; i--) {
                    if (!logList[i].includes('-')) {
                        logList.splice(i, 1); // Remove the element at index i
                    }
                }
                ...
                logList.forEach(filename => {
                    ...
                    _log.eliminar(filename + '.gz.b64', (err) => {
                        ...
                        if (index == logList.length) {
                            ...
                            if (errores == 0) {
                                ...
                                console.log('All log files were deleted');
                                interface.removeListener('line', deleteLogsListener);
                                interface.on('line', lineListener);
                                interface.prompt();

                            } else {

                                console.log('All logs might not be totally deleted');
                                interface.removeListener('line', deleteLogsListener);
                                interface.on('line', lineListener);
                                interface.prompt();
                ...
                console.log('There was an error while trying to access log data');
                interface.removeListener('line', deleteLogsListener);
                interface.on('line', lineListener);
                interface.prompt();
        ...
        console.log('The logs were not deleted');
        interface.removeListener('line', deleteLogsListener);
        interface.on('line', lineListener);
        interface.prompt();
...
let deleteUserListener = function (str) {
                                ...
                                _data.delete('checks', checks[index], (err) => {
                                    ...
                                    if (index == aBorrar - 1) {
                                        ...
                                        if (errores == 0) {
                                            ...
                                            interface.removeListener('line', deleteUserListener);
                                            interface.on('line', lineListener);
                                            interface.prompt();

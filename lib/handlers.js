...
//Pagina con el formulario para crear checks
...
handlers.createCheck = function (data, callback) {
    ...
    if (data.metodo == 'get') {
        ...
        var dinamic_content = {
            ...
            'head_titulo': 'New Check',
            'body_class': 'createCheck',
            'hoja_estilos': 'public/createCheck.css'

        }

        helpers.prepareTemplate('createCheck', dinamic_content, function (err, str) {
...
//Pagina con el formulario para editar checks

handlers.editCheck = function (data, callback) {

    if (data.metodo == 'get') {

        var dinamic_content = {

            'head_titulo': 'Edit Check',
            'body_class': 'editCheck',
            'hoja_estilos': 'public/editCheck.css'

        }

        helpers.prepareTemplate('editCheck', dinamic_content, function (err, str) {
...
handlers._checks.post = function (data, callback) {
    ...
    let url = typeof (data.contenido.url) == "string" && data.contenido.url.trim().length >= 1 ? data.contenido.url.trim() : "";
    ...
    if (protocolo && url.length > 0 && timeoutSeconds && validCodes && metodo) {
        ...
        handlers._tokens.verSesion(sesion, (numero) => {
            ...
            if (numero) {
                ...
                _data.read('users', numero, (err, usuario) => {
                    ...
                    if (!err && usuario) {
                        ...
                        if (checks.length < config.maxChecks) {
                            ...
                            //We check first to make sure the url is actually valid and has DNS records

                            let parsedUrl = new URL(protocolo + '://' + url);
                            ...
                            dns.resolve(hostname, (error, records) => {
                                ...
                                if (!error && records) {
                                        ...
                                        if (!err) {
                                            ...
                                            _data.update("users", numero, usuario, (err) => {
                                                ...
                                                if (!err) {
                                                    ...
                                                    debug('No se pudo actualizar el usuario ' + numero);
                                            ...
                                            debug('No se pudo guardar el nuevo check por un error');
                                    ...
                                    debug('No se encontro registro DNS para la url: '+url);

                                    callback(400, {'Error':'No se encontro registro DNS para esta url, por lo tanto no se creo el check'});
                            ...
                            debug('Ya se alcanzo el numero maximo de checks por usuario');
                        ...
                        debug('No se encontro el usuario ' + numero);
                        debug(err);
                ...
                callback(403, { 'Error': 'No tiene una sesion activa, pruebe a iniciar sesion nuevamente' });
        ...
        callback(400, { 'Error': 'No se enviaron todos los datos requeridos' });
...
handlers._checks.get = function (data, callback) {
    ...
    let checkId = typeof (data.url.searchParams.get('checkId')) == "string" && data.url.searchParams.get('checkId').trim().length === 20 ? data.url.searchParams.get('checkId').trim() : false;
    let token = typeof (data.headers.token) == 'string' && data.headers.token.trim().length == 20 ? data.headers.token.trim() : false;
...
handlers._checks.delete = function (data, callback) {
    ...
    if (checkId) {
        ...
        callback(400, { 'Error': 'No se envio un id de check valido' });

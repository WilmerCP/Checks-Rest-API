...
helpers.convertToJSON = function (texto) {
    ...
    try {
        ...
        if(typeof(texto) == 'number'){
            ...
            throw new Error('Numbers are not accepted while parsing to JSON');
...
helpers.bringTemplate = function (nombreArchivo, callback) {
    ...
    let directorio = path.join(__dirname, '..', 'plantillas');
...
//Recuperar archivos estaticos para servirlos al usuario

helpers.getStaticAsset = function(nombreArchivo,callback){

    let directorio = path.join(__dirname, '..', 'public');

    var tipo = nombreArchivo.includes('.css') || nombreArchivo.includes('.js') ? 'utf8' : null;

    if(typeof(nombreArchivo == 'string') && nombreArchivo.length > 0){

        fs.readFile(directorio + '/' + nombreArchivo,tipo,(err,data)=>{
        ...
        callback('No se solicito un nombre de archivo valido');
